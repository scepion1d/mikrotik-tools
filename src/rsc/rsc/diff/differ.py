...
def _strip_identity(props: dict[str, str]) -> dict[str, str]:
    ...
    Used by :mod:`rsc.diff.verify`'s :func:`menu_signature` to compute a
...
def _normalise_value(value: str | None) -> str | None:
    ...
    2. **IP address text form.** Authored ``::ffff:0:0/96`` and
       /export's ``::ffff:0.0.0.0/96`` are the same prefix; ``2001:DB8::1``
       and ``2001:db8::1`` are the same address. Each comma- or
       dash-separated token is run through :mod:`ipaddress` and emitted
       in the canonical compressed form. Tokens that don't parse as an
       address are left untouched, so port lists (``53,67``), interface
       lists, and arbitrary strings are unaffected.
    """
    if value is None:
        return None
    if (
...
def _canonicalise_addresses(value: str) -> str:
    ...
    interface (with ``/prefix``) or a bare address; on success emits the
    canonical compressed form. Anything that doesn't parse is returned
    unchanged -- this keeps non-address values (ports, names, MACs,
    booleans, comments) untouched.
    """
...
def _try_canon_one(token: str) -> str | None:
    """Return the canonical form of a single IP token, or None if it isn't one.
