...
MENU_COMPUTED: dict[str, frozenset[str]] = {
    ...
    # /ip/dhcp-client `name` is auto-assigned by RouterOS as `clientN`
    # (positional counter) when the row is added without an explicit
    # name=. We never author it (the iac.dhcpc.* identity lives in
    # `comment=`), but /export emits it. Without this entry the differ
    # produces phantom `reset name` (rollforward) / `set name=clientN`
    # (rollback) ops on every deploy. The actual identity for matching
    # this menu's rows is the comment token, not name.
    "/ip/dhcp-client": frozenset({"name"}),
}
