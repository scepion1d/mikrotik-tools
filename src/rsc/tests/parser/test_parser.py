...
# Make `rsc.parser` importable when running this file directly with `python`.
sys.path.insert(0, str(Path(__file__).resolve().parents[2]))

from rsc.parser import (  # noqa: E402
    Config,
    Item,
    entity_id,
    is_synthetic,
...
def test_set_with_padded_find_selector_surfaces_kv() -> None:
    """``[ find KEY=VAL ]`` (with spaces) surfaces KEY=VAL like the bare form.

    /export emits the padded form; the bundler emits the bare form. Both
    must surface the selector's KEY=VAL into props or the differ sees
    phantom drift between live and candidate (regression: ``/ipv6/nd``
    rolled-back as ``set default=no`` because only the bare form surfaced
    ``default=yes``).
    """
    bare = parse_text(
        "/ipv6/nd\n"
    ...
    bare_props = bare.items_by_menu["/ipv6/nd"][0].props
    padded_props = padded.items_by_menu["/ipv6/nd"][0].props
    # Both must surface `default=yes` from the selector.
    assert bare_props.get("default") == "yes", bare_props
    assert padded_props.get("default") == "yes", padded_props
    # The non-selector content must agree.
    assert bare_props["advertise-dns"] == padded_props["advertise-dns"] == "yes"
