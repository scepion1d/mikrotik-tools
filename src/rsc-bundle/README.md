...
Bundles a flat RouterOS `.rsc` profile folder (`rsc/<profile>/{secrets.rsc, vars.rsc, NN-*.rsc}`) into one minimal deploy-ready file. Substitutes `:global` vars, strips RouterOS scripting, parses with [`rsc-parser`](../rsc-parser/), and re-emits one operation per line. Properties (including comments) are preserved verbatim.
...
```text
usage: rsc-bundle [-h] --profile PROFILE [-o OUT] [--no-flatten]

Bundle a flat RouterOS .rsc profile folder into one minimal deploy-ready file.
By default, $var references are substituted and scripting wrappers are
stripped; properties (including comments) are preserved verbatim.
...
- Profile folder must be **flat** — subdirectories are not traversed.
